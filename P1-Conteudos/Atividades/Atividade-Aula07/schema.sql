...
-- Criar uma consulta que conte quantos usuários estão cadastrados (usuario)
SELECT COUNT(*) AS NUMERO_DE_USUARIOS FROM USUARIO;

-- Criar uma consulta que conte quantos eventos existem de cada tipo (evento por id_tipo_evento).
SELECT idtipoevento AS Tipo_Evento, COUNT(*) FROM EVENTO GROUP BY idtipoevento;

-- Criar uma consulta que mostre o evento mais antigo e o mais recente.
SELECT MIN(datahora) AS evento_mais_antigo, MAX(datahora) AS evento_mais_recente FROM EVENTO;

-- Criar uma consulta que calcule a média de registros por cidade (localização).
SELECT AVG(total) AS Média_por_cidade 
	FROM (
